...
class Rps extends BaseController
{
	...
	public function mingguan_create($rps_id)
	{
		...
		$mingguanModel = new RpsMingguanModel();
		$existingWeeks = $mingguanModel->where('rps_id', $rps_id)
			->select('minggu')
			->findColumn('minggu');
	...
	public function mingguan_store($rps_id)
	{
		...
		$duplikatMinggu = $mingguanModel->where('rps_id', $rps_id)
			->where('minggu', $minggu)
			->first();
		if ($duplikatMinggu) {
	...
	public function mingguan_edit($id)
	{
		...
		// Get existing week numbers (excluding current)
		$existingWeeks = $mingguanModel->where('rps_id', $rencana_mingguan['rps_id'])
			->where('id !=', $id)
			->select('minggu')
			->findColumn('minggu');
	...
	public function mingguan_update($id)
	{
		...
		// Check for duplicate week number (excluding current record)
		$duplikatMinggu = $mingguanModel->where('rps_id', $rps_id)
			->where('id !=', $id)
			->where('minggu', $minggu)
			->first();
