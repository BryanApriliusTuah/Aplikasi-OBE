...
<script>
	...
	const _CPMK_STORAGE_KEY = 'capaian_cpmk_state';

	function _cpmkSaveState() {
		try {
			localStorage.setItem(_CPMK_STORAGE_KEY, JSON.stringify({
	...
	function _cpmkRestoreState() {
		try {
			const raw = localStorage.getItem(_CPMK_STORAGE_KEY);
			if (!raw) return;
	...
	window.addEventListener('beforeunload', _cpmkSaveState);
</script>
