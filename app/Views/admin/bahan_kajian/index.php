    ...
    <div class="p-0">
        <?php if (empty($bahan_kajian)): ?>
            <div class="text-center text-muted py-5">
                ...
                <p class="small">Silakan tambah data bahan kajian</p>
            </div>
        <?php else: ?>
            <div class="modern-table-wrapper">
                <div class="scroll-indicator"></div>
                <table class="modern-table" id="bahanKajianTable">
                    <thead>
                        <tr>
                            <th class="text-center" style="width: 50px;">No</th>
                            <th class="text-center" style="width: 150px;">Kode BK</th>
                            <th class="text-start">Nama Bahan Kajian</th>
                            <?php if (session('role') === 'admin'): ?>
                    ...
                    <tbody>
                        <?php $no = 1; foreach ($bahan_kajian as $b): ?>
                            <tr>
                                ...
                                <td class="text-center"><?= esc($b['kode_bk']) ?></td>
                                <td style="white-space: normal;"><?= esc($b['nama_bk']) ?></td>
                                <?php if (session('role') === 'admin'): ?>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center align-items-center gap-1">
                                            <a href="<?= base_url('admin/bahan-kajian/edit/' . $b['id']) ?>" class="btn btn-sm btn-outline-primary" title="Edit">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <button type="button"
                                                class="btn btn-sm btn-outline-danger btn-hapus"
                                                data-id="<?= $b['id'] ?>"
                                                data-kode="<?= esc($b['kode_bk']) ?>"
            ...
            <div id="bahanKajianTable_pagination" class="d-flex justify-content-between align-items-center mt-3 px-3 pb-3"></div>

            <div class="card-footer bg-light border-0 py-3">
                <div class="d-flex align-items-center gap-3 justify-content-center">
                    <small class="text-muted">
                        <i class="bi bi-info-circle me-1"></i>
                        Total: <?= count($bahan_kajian) ?> bahan kajian
                    </small>
                </div>
            </div>
        <?php endif; ?>
        ...
        <form id="formHapus" action="" method="post">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Yakin ingin menghapus data <strong id="modalKodeBK"></strong>?</p>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    modalHapus.addEventListener('show.bs.modal', function (event) {
        ...
        formHapus.action = "<?= base_url('admin/bahan-kajian/delete/') ?>" + id;
    ...
    initPagination('bahanKajianTable', 10);
});
</script>
<?= $this->endSection() ?>
