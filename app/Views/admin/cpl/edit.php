    ...
    <div class="row justify-content-center">
        <div class="col-lg-6 col-md-10">
            <div class="card shadow-sm mt-4">
                <div class="card-body">
                    <h3 class="fw-bold mb-4">Edit CPL</h3>
                    <a href="<?= base_url('admin/cpl') ?>" class="btn btn-outline-secondary mb-3">
                        <i class="bi bi-arrow-left"></i> Kembali
                    </a>

                    <?php
                        // Ekstrak 2 digit angka dari kode lama, mis: "CPL07" -> "07"
                        $angka = preg_replace('/^\s*CPL/i', '', $cpl['kode_cpl'] ?? '');
                    ?>

                    <form action="<?= base_url('admin/cpl/update/' . $cpl['id']) ?>" method="post" autocomplete="off">
                        <?= csrf_field() ?>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control <?= (isset($validation) && $validation->hasError('deskripsi')) ? 'is-invalid' : '' ?>" id="deskripsi" name="deskripsi" rows="3" required><?= esc(old('deskripsi', $cpl['deskripsi'])) ?></textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= (isset($validation) && $validation->hasError('jenis_cpl')) ? 'is-invalid' : '' ?>" id="jenis_cpl" name="jenis_cpl" required>
                                <option value="">-- Pilih --</option>
                                <option value="S"  <?= old('jenis_cpl', $cpl['jenis_cpl'])=='S'  ? 'selected' : '' ?>>S (Sikap)</option>
                                <option value="P"  <?= old('jenis_cpl', $cpl['jenis_cpl'])=='P'  ? 'selected' : '' ?>>P (Pengetahuan)</option>
                                <option value="KK" <?= old('jenis_cpl', $cpl['jenis_cpl'])=='KK' ? 'selected' : '' ?>>KK (Keterampilan Khusus)</option>
                                <option value="KU" <?= old('jenis_cpl', $cpl['jenis_cpl'])=='KU' ? 'selected' : '' ?>>KU (Keterampilan Umum)</option>
                            </select>
