	...
	<div class="mb-2">
		...
		<select name="cpl_id" id="select-cpl" class="form-control" required>
			...
			<?php foreach ($cpl as $c) : ?>
				<option value="<?= $c['id'] ?>" data-deskripsi="<?= esc($c['deskripsi'] ?? '') ?>" <?= old('cpl_id') == $c['id'] ? 'selected' : '' ?>><?= esc($c['kode_cpl']) ?></option>
			<?php endforeach ?>
		</select>
...
<script>
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		function populateDropdown(dropdown, options, placeholder) {
			...
			if (Array.isArray(options) && options.length > 0) {
				options.forEach(option => {
					...
					if (option.deskripsi) opt.dataset.deskripsi = option.deskripsi;
		...
		setupDeskripsiDisplay(cplDropdown, 'cpl-deskripsi');
		setupDeskripsiDisplay(cpmkDropdown, 'cpmk-deskripsi');
		setupDeskripsiDisplay(subcpmkDropdown, 'subcpmk-deskripsi');
	});
</script>
<?= $this->endSection() ?>